...
cmd_status() {
    ...
    if ! is_access_token_valid; then
        ...
        if [ -n "$REFRESH_TOKEN" ] && ! is_refresh_token_valid; then
            ...
            echo "Refresh token might still be valid (Expires at: $(date -d "@$RT_EXPIRES_AT" -u --iso-8601=seconds)). Try calling an API to trigger refresh or login."
        fi
        return 1
    fi
    echo "Status: Active session."
    echo "Access Token Expires At: $(date -d "@$AT_EXPIRES_AT" -u --iso-8601=seconds) (in $((AT_EXPIRES_AT - $(date +%s))) seconds)"
    echo "Refresh Token Expires At: $(date -d "@$RT_EXPIRES_AT" -u --iso-8601=seconds) (in $((RT_EXPIRES_AT - $(date +%s))) seconds)"
