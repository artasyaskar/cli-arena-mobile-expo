...
generate_token() {
    ...
    local token_value="${type_prefix}${user_id}_$(head /dev/urandom | tr -dc A-Za-z0-9 | head -c 10)_${expiry_timestamp}"
    echo "$token_value"
}
...
handle_login() {
    ...
    if [ -p /dev/stdin ]; then
        ...
        if echo "$body" | jq -e . >/dev/null 2>&1; then # JSON body
            ...
            username=$(echo "$body" | grep -oP '(?<=username=)[^&]*')
            password=$(echo "$body" | grep -oP '(?<=password=)[^&]*')
        fi
