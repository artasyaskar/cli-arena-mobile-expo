...
MOCK_AUTH_API_SCRIPT="./resources/mock_auth_api.sh" # Path to the auth API mock
...
handle_request() {
    ...
    # Pass token via env var REQ_ACCESS_TOKEN for mock_auth_api.sh to pick up
    local validation_response
    validation_response=$(REQ_ACCESS_TOKEN="$token" "$MOCK_AUTH_API_SCRIPT" /validate)
